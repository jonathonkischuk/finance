...
def put_option_price(S, E, T, rf, sigma):
    # first we have to calculates s1 and d2 parameters
    d1 = (log(S / E) + (rf + sigma * sigma / 2.0) * T) / (sigma * sqrt(T))
    d2 = d1 - sigma * sqrt(T)
    print("The d1 and d2 parameters: %s, %s" % (d1, d2))
    # use the N(x) to calculate the price of the option
...
if __name__ == "__main__":
    ...
    print("Call option price accordingg to Black Scholes model: ",
          call_option_price(S0, E, T, rf, sigma))

    print("put option price accordingg to Black Scholes model: ",
